...
# ===============================================================================================
def simulate_prices_and_payoff(
    df, strike, num_paths=15000, nPeriods=720, cap_level=0.3, risk_free_rate=0.05
):
    ...
    def mrjpdf(params, Pt, Pt_1):
        ...
        term1 = (
            lambda_
            * np.exp(-((Pt - a - phi * Pt_1 - mu_J) ** 2) / (2 * (sigmaSq + sigmaSq_J)))
            / np.sqrt(2 * np.pi * (sigmaSq + sigmaSq_J))
        )
        term2 = (
            (1 - lambda_)
            * np.exp(-((Pt - a - phi * Pt_1) ** 2) / (2 * sigmaSq))
            / np.sqrt(2 * np.pi * sigmaSq)
        )
        return term1 + term2
    ...
    for i in range(nPeriods):
        trend = (
            final_trend_value  # Use the final trend value for all future time points
        )
        SimLogPricesWithTrend[i, :] = (
            logSimPrices[i, :] + trend + stochastic_trend[i, :]
        )
    simulated_log_df = pd.DataFrame(
        SimLogPricesWithTrend,
        index=SimPriceDates,
        columns=[f"Trial_{i+1}" for i in range(num_paths)],
    )
    ...
    # ===============================================================================================
    payoffs = np.maximum(
        np.minimum((1 + cap_level) * strike, final_prices_twap) - strike, 0
    )
    ...
    plt.plot(
        simulated_df_twap.index,
        simulated_df_twap.iloc[:, 0],
...
df = pd.read_csv(
    "../data.csv"
)  # Ensure your data.csv has 'timestamp' and 'base_fee' columns
